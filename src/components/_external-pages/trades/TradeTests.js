...
export default function ComponentCourses(props) {
  ...
  const [category, setCategory] = React.useState("operative");
  ...
  useEffect(() => {
    ...
    if (
      ...
      switch (queryCategory) {
        ...
        case "managers-and-professional":
          setValue(1);
          break;
  ...
  const handleChange = (event, newValue) => {
    switch (newValue) {
      case 0:
        setCategory("operative");
        break;

      case 1:
        setCategory("managers-and-professional");
        break;
  ...
  return (
    <Grid id="csl-tests" container spacing={3} sx={{ my: 10 }}>
              ...
              {props.tests?.length ? (
                props.tradeId ? (
                  props.tests?.map((item) => (
                    <TestsCard key={item.name} {...item} />
                  ))
                ) : (
                  props.tests
                    ?.filter((item) => item.category === category)
                    ?.map((item) => <TestsCard key={item.name} {...item} />)
                )
              ) : (

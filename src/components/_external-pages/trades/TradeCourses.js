...
const tabsOption = [
  ...
  {
    label: "Managerial and Directoral",
...
export default function ComponentCourses(props) {
  ...
  useEffect(() => {
    ...
    if (
      ...
      switch (queryCategory) {
        ...
        case "managerial-and-directoral":
          setValue(1);
          break;
  ...
  const handleChange = (event, newValue) => {
    switch (newValue) {
      ...
      case 1:
        setCategory("managerial-and-directoral");
        break;
              ...
              {props.courses?.length ? (
                props.tradeId ? (
                  props.courses?.map((item) => (
                    <CoursesCard key={item.name} {...item} />
                  ))
                ) : (
                  props.courses
                    ?.filter((item) => item.category === category)
                    ?.map((item) => <CoursesCard key={item.name} {...item} />)
                )
              ) : (
